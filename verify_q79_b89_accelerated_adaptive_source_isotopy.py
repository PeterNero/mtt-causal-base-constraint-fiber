#!/usr/bin/env python3
"""Independently verify a recursive B89 source-isotopy result shard."""
...
WORKER = HERE / "q79_b89_accelerated_adaptive_source_isotopy_worker.py"
CELL_WORKER = HERE / "q79_b89_accelerated_source_isotopy_worker.py"
...
def independent_binding(previous_tubes: list[dict], current_tubes: list[dict]) -> dict:
    ...
    require(set(previous) == set(current) == set(range(252)), "endpoint labels")
    count = 0
...
def main() -> None:
    ...
    baseline_path = Path(args.baseline_root).resolve() / BASELINE_RELATIVE
    ...
    checks["schema_and_tier"] = (
        ...
        and packet["tier"]
        == "CERTIFIED_RECURSIVE_DYADIC_COMPLETE_SOURCE_TUBES_AND_GUIDE_ISOTOPY"
    )
    ...
    checks["source_and_guides_are_hash_bound"] = (
        ...
        and packet["reference_artin_sha256"] == metadata["source"]["artin_sha256"]
    ...
    checks["numerical_policy_is_selected_policy"] = (
        packet["branch_range"] == [0, 252]
        and packet["precision_bits"] == 512
    ...
    cell_worker_hash = sha256(CELL_WORKER)
    ...
    for offset, (row, attempt_entry) in enumerate(
        ...
        for index, subcell in enumerate(subcells):
            require(subcell["certified_branches"] == 252, "complete subcell carrier")
            require(len(subcell["tubes"]) == 252, "complete tube list")
            require(
                {int(tube["branch"]) for tube in subcell["tubes"]} == set(range(252)),
                "complete tube labels",
            )
            ...
            accelerated = subcell["accelerated_pair_certificate"]
            require(accelerated["wrapper_sha256"] == cell_worker_hash, "cell worker hash")
            require(
                accelerated["baseline_certifier_sha256"] == baseline_hash,
                "baseline certifier hash",
            )
            require(accelerated["certified_cell_calls"] == 1, "one cell call")
            expected_binding = (
    ...
    checks["declared_counts_replay"] = (
        ...
        and counts["certified_branch_tubes"] == total_subcells * 252
