#!/usr/bin/env python3
"""Recursively certify difficult B89 source-isotopy cells.
...
CELL_CERTIFIER = HERE / "q79_b89_accelerated_source_isotopy_worker.py"
...
def child_command(
    ...
    return [
        ...
        str(interval + 1),
        "--branch-start",
        str(args.branch_start),
        "--branch-stop",
        str(args.branch_stop),
        "--precision",
        str(args.precision),
        "--predictor-degree",
...
def validate_child(
    ...
    require(packet["branch_range"] == [args.branch_start, args.branch_stop], "child branch range")
    ...
    require(
        row["certified_branches"] == args.branch_stop - args.branch_start,
        "child carrier",
    )
    require(
        packet.get("accelerated_pair_certificate", {}).get("certified_cell_calls") == 1,
        "accelerated child pair certificate",
    )
    row["accelerated_pair_certificate"] = packet["accelerated_pair_certificate"]
    return row
...
def build_checkpoint_packet(
    ...
    return {
        ...
        "tier": "CERTIFIED_RECURSIVE_DYADIC_COMPLETE_SOURCE_TUBES_AND_GUIDE_ISOTOPY",
        "source_sha256": sha256(source_path),
        "guide_payload_sha256": sha256(guides_path),
        "guide_metadata_sha256": sha256(metadata_path),
        "reference_artin_sha256": metadata["source"]["artin_sha256"],
        ...
        "branch_range": [args.branch_start, args.branch_stop],
        ...
        "recursive_dyadic_policy": {
            ...
            "every_leaf_has_an_independent_accelerated_cell_certificate": True,
        ...
        "counts": {
            ...
            "certified_branch_tubes": selected_subcells * 252,
        ...
        "checks": {
            ...
            "every_selected_subcell_has_all_declared_source_tubes": True,
            ...
            "the_guide_payload_and_reference_Artin_word_are_hash_bound": True,
        ...
        "guardrails": {
            ...
            "claims_the_boundary_or_mixed_carrier_is_certified_here": False,
...
def main() -> int:
    ...
    parser.add_argument("--branch-start", type=int, default=0)
    parser.add_argument("--branch-stop", type=int, default=252)
    ...
    require(args.branch_start == 0 and args.branch_stop == 252, "complete carrier")
    ...
    attempts = []
    previous_tubes = None
    with tempfile.TemporaryDirectory(prefix="mtt-accelerated-adaptive-") as temporary:
    ...
    selected_subcells = sum(len(row["subcells"]) for row in logical_rows)
    print(
        "accelerated adaptive source-isotopy shard PASS "
