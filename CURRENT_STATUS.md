...
`EquivariantFeshbachOneDimensionalResponseTheorem.v1` now proves the exact
covariance statement needed after CBF.T18 and also closes a tempting false
shortcut. Both reducing compression and the nonreducing Feshbach operator
preserve every exact symmetry intertwined by the synthesis map; the inverse
of the excluded block is equivariant whenever it exists. However, the
source-pinned phase/shift parity and Fourier lane exchange leave a
nine-dimensional Hermitian response module, not the line spanned by
`H_resp`. An exact `I6` Feshbach negative control has all those symmetries and
is not response-proportional.

The finite physical reduction ladder is now exact:

```text
four gauge-sector Herm(3) blocks                         36
Fourier-paired quark/lepton blocks                       18
one universal routed Herm(3) block                        9
selected relative-response source line                    1
```

On the active family-lane carrier the comparison algebra generated by
`diag(A,A)`, `diag(B,B)`, lane parity and Fourier lane exchange has scalar
commutant. Since `H_resp,act` is invertible, the one-dimensional endpoint
condition is equivalently

```text
T_rel=H_resp,act^-1 H_eff,act
commutes with the selected comparison algebra.
```

This replaces a general matrix-fit problem by support/multiplicity checks, one
same-root relative source intertwiner and one scalar evaluation. It does not
...
`CausalRelativeCauchyEvolutionAndStateSeparatedGlobalG0Theorem_v1.md`
(`CBF.T44`) closes the state-free global causal-evolution layer of the direct
route. For compactly supported order-zero changes `D_h=D_H+V` of the T25
Green-hyperbolic product Dirac operator, it proves the exact Moller identities

```text
M_h=1-E_h V,
M_h^(-1)=1+E_H V,
M_h M_h^(-1)=M_h^(-1) M_h=1.
```

Advanced/retarded comparison yields relative Cauchy evolution on the solution
quotient and the representation-independent even CAR algebra. The existing
renormalized relative S-matrix then emits the global local-formal contour
element
...
The minimal two-leg cellular boundary has integral kernel generated by
`(1,-1)`. Normalizing the forward coefficient to one therefore selects that
primitive return chain uniquely, with no continuous contour parameter. A
common central determinant phase cancels from `C_H`, while source-dependent
relative phase remains typed and is not discarded.

Scalarization requires a state:

```text
Z_omega=omega(C_H).
```

An exact `M_2(C)` witness produces `(3+4i)/5`, `(3-4i)/5` and `3/5` from
three positive normalized states on the same unequal-source operator, although
all return one at equal sources. Thus the return identity does not select a
preferred state. T43 remains the anchored local one-loop reference and its
direct source root is hash-matched, but no equality with every global in-in
functional is claimed.

The old phrase "missing global domain, cycle and common phase" is now too
coarse for the direct route. Those subclauses are closed or bypassed at the
state-free operator tier. The remaining physical scalar `G0` obligation is
the selected state/initial functional, relative phase and interacting
fixed-coupling removal, so scalar `G0` and `G2` are interlocked. The internal
shared circle remains distinct from Lorentzian time and does not by itself
select the contour. Physical counters remain `0/3` gates, `0/3` packets and
...
The cumulative requester-side audit now accepts 87 campaign capsules: all 65
boundary packets and 22 branch packets. Every accepted capsule passed archive,
...
```text
boundary carrier: 2195/2195, complete
branch carrier:     642/2195, incomplete
```
...
exact branch gaps are edge 1 `[324,336)`, `[349,361)`, `[409,456)` and
`[480,857)`, edge 2 `[1,678)`, and edge 3 `[1,429)`, totaling 1,553
intervals. A fifth
...
independent-verifier gate, followed by `[468,480)` under the same rule and a
normally completed `[456,468)` capsule. At this snapshot 11 jobs are running,
2 are claimed and 118 are queued; process state remains non-evidence until
ingested.
