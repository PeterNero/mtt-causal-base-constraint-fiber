...
MODULE_PATH = ROOT / "q79_b89_accelerated_adaptive_source_isotopy_worker.py"
SPEC = importlib.util.spec_from_file_location("adaptive_source_isotopy", MODULE_PATH)
...
def row(start, stop):
    return {
        ...
        "tubes": [tube(label, float(start) + label, float(stop) + label) for label in range(252)],
    }


class Q79B89AcceleratedAdaptiveSourceIsotopyTests(unittest.TestCase):
    def test_endpoint_binding_is_identity_and_complete(self):
        previous = [tube(label, label - 1, label) for label in range(3)]
        current = [tube(label, label, label + 1) for label in range(3)]
        binding = MODULE.endpoint_binding(previous, current)
        self.assertEqual(
            binding,
            {
                "bound_branches": 3,
                "unique_label_matches": 3,
    ...
    def test_atomic_checkpoint_declares_only_certified_prefix(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            packet = MODULE.build_checkpoint_packet(
                args,
                {"source": {"artin_sha256": "abc"}, "edge": 1},
                source,
                guides,
                metadata_path,
                [logical],
